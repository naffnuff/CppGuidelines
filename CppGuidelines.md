...
This document focuses on the many subtle yet avoidable pitfalls and hazards that often plague C++ code. The format is a series of judgments and their motivations, that should be at least food for thought for any C++ programmer, even if not abided by in all instances.

Decisions on formatting, naming and other more or less arbitrary choices should be kept in a different document.
